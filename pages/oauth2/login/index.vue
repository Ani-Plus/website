        ...
        <div class="hero min-h-screen bg-base-200" style="background-image: url(https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/218894fb-2ed2-4a32-b4dd-7e017fccd106/df2b9e6-b4be8ccf-a37c-400d-acf9-458c8be1924e.jpg/v1/fill/w_1280,h_720,q_75,strp/4k_anime_wallpaper_for_pc_by_rockydevilweeb_df2b9e6-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..TsVYlRixl5gXD50UA1aPVY9Jb5BuPiKi3m3dWIdoILs);">
            ...
            <div class="hero-content flex-col lg:flex-row-reverse">
                <div class="text-center lg:text-center">
                    ...
                    <p class="py-6">Ani+ sistemlerinden olan liste oluşturma, düzenleme, silme; anime ekleme, silme; bölüm ekleme, düzenleme, silme; istatistiklere erişim; profil düzenleme, görüntüleme gibi özelliklere etişmek için giriş yapın!</p><br>
                    <h1 class="text-3xl">Hesabın yok mu?</h1><br>
                    <button class="btn btn-primary" @click="redirect('register')">Kayıt Ol!</button>
                </div>
                <br>
                <div class="card flex-shrink-0 w-full max-w-sm bg-base-100">
                    <div class="card-body">
                        <form name="login" action="/">
                            <div class="form-control">
                                <label class="label">
                                    <span class="label-text">Kullanıcı ID</span>
                                </label>
                                <input name="userid" type="text" placeholder="Kullanıcı ID" class="input input-bordered" />
                            </div>
                            <div class="form-control">
                                <label class="label">
                                ...
                                <label class="label">
                                    <a href="https://discord.com/users/539843855567028227" class="label-text-alt link link-hover">Kullanıcı ID'nizi veya şifrenizi mi unuttunuz? Bizimle İletişime Geçin!</a>
                                </label>
                            </div>
        ...
        async fetch() {
            ...
            if(process.server) {
                ...
                if(!this.$route.query.username) {
                    //kayıt işlemi gerçekleştirilmemiş
                }else{
                    //kayıt işlemi gerçekleştirilmiş
                    const ids = YAML.parse(fs.readFileSync(path.join(process.cwd(), `/database/profiles/ids.yaml`), 'utf-8')).ids
                    var lastid = ids[ids.length - 1]
                    var nozero = String(parseInt(lastid) + 1)
                    var newid = String(nozero.padStart(6, "0"))
                    ids.push(newid)
                    fs.writeFileSync(path.join(process.cwd(), `/database/profiles/${String(newid)}.yaml`), `username: "${this.$route.query.username}"\nuserid: "${newid}"\nuserpassword: "${this.$route.query.userpassword}"\nstatus: "offline"\nuseravatar: "https://cdn.discordapp.com/attachments/775822548519616562/1016221074104528906/defaultgreen.png"\nisMod: false\nuploadedEpisodes: 0\naddedAnimes: 0\nlistsCount: 0\nlists: []\nanimes: []`)
                    fs.writeFileSync(path.join(process.cwd(), '/database/profiles/ids.yaml'), "ids:\n" + YAML.stringify(ids))
                }
